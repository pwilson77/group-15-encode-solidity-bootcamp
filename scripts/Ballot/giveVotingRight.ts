...
async function main() {
  ...
  if (chairpersonAddress !== signer.address)
    throw new Error("Caller is not chairperson for this contract");
  console.log(`Giving right to vote to ${voterAddress}`);
  const give = await ballotContract.giveRightToVote(voterAddress);
  console.log("Awaiting confirmations");
  await give.wait();
  console.log(`Transaction completed. Hash: ${give.hash}`);
}
